...
CLIMA_API_URL = "http://localhost:8000/clima?cidade={}"

# Função para verificar condições climáticas adversas
def verificar_condicoes_climaticas(cidade):
    try:
        response = requests.get(CLIMA_API_URL.format(cidade))
        if response.status_code == 200:
            data = response.json()
            clima_info = data["clima"]
            
            if "Rain" in clima_info or "Storm" in clima_info:
                return True  # Chuva ou tempestade
            
            temperatura = int(clima_info.split("+")[1].split("°")[0])
            if temperatura > 35 or temperatura < 5:
                return True  # Temperatura extrema
            
        return False
    except Exception as e:
        print(f"Erro ao obter informações do clima: {e}")
        return False

@when('um usuário solicita os ganhos do entregador "{entregador_id}"')
def step_impl(context, entregador_id):
    if context.api_available:
        ...
        if response.status_code == 200:
            ...
            if context.clima_adverso:
                context.ganho_bruto *= 1.10  # Aplica o aumento de 10%
            context.saldo_final = context.ganho_bruto
        else:
            context.saldo_final = None
            context.ganho_bruto = 0  # Garante um valor padrão
    else:
        context.saldo_final = None
        context.ganho_bruto = 0  # Garante um valor padrão


@given('a cidade "{cidade}" tem clima adverso')
def step_impl(context, cidade):
    if cidade == "São Paulo":
        context.clima_adverso = True  # Garantir que clima seja adverso para o teste
    else:
        context.clima_adverso = False


@then(u'o valor exibido na UI deve ser aumentado em 10%')
def step_impl(context):
    if context.clima_adverso:
        assert context.saldo_final == context.ganho_bruto * 1.10, f"A taxa não deveria ter aumentado. Esperado: {context.ganho_bruto * 1.10}, Obtido: {context.saldo_final}"
    else:
        assert context.saldo_final == context.ganho_bruto, f"A taxa não deveria ter aumentado. Esperado: {context.ganho_bruto}, Obtido: {context.saldo_final}"
...
@when('um usuário solicita os ganhos do entregador "{entregador_id}" verificando o cache')
async def step_impl(context, entregador_id):
    redis_conn = await get_redis_connection()
